...
MapQuad::MapQuad(MapQuad* parent, float*** vertexes, glm::vec3*** normals, int height, int width, int offsetX, int offsetY)
	: Entity(generateMesh(width, height, vertexes, normals, offsetX, offsetY)),
		texture(Engine::getInstance().getTextureManager()->getTexture("assets/textures/ground1.png")),
	...
	glm::mat3 t = {1.f, 2.f, 3.f, 4.f, 5.f, 6.f, 7.f, 8.f, 9.f};
	t = glm::transpose(t);
}
...
void MapQuad::setRadius() { // Used for the culling process, if sphere generated by the center and the radius intersects with the frustrum the quad will be
														// rendered
	radius = 0.0;
...
void MapQuad::setVertexValue(int x,
														 int y,
														 float val) { // can manipulate the height of a vertex, a call to MapTools::calculateNormals() should be performed afterwards
	*vertexes[x][y] = val;
}
